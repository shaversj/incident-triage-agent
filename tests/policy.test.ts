...
test("read-only approval-required decisions omit simulated action state", () => {
  const package_ = packageFor("bad-deploy-latency");
  const validation = parseDecisionText(JSON.stringify({
    incident_class: "bad_deploy",
    next_action: "request_rollback_approval",
  ...
  expect(validation.decision).toBeDefined();
  const safety = evaluateSafety(validation.decision!, package_, { mode: "read_only" });

  expect(safety.status).toBe("approval_required");
  expect(safety.stagedPayload).toBeUndefined();
  expect(safety.auditEvent).toBeUndefined();
  expect(safety.mitigationControl.dryRun).toBeUndefined();
  expect(safety.mitigationControl.stagedAction).toBeUndefined();
  expect(safety.mitigationControl.approvalRequest).toBeUndefined();
});
